...
public:
    inline SHESDParams alpha(float alpha) {
        this->alpha_ = alpha;
        return *this;
    };

    inline SHESDParams max_anoms(float max_anoms) {
        this->max_anoms_ = max_anoms;
        return *this;
    };

    inline SHESDParams direction(Direction direction) {
        this->direction_ = direction;
        return *this;
    };

    inline SHESDParams verbose(bool verbose) {
        this->verbose_ = verbose;
        return *this;
    };

    inline SHESDParams callback(std::function<void()> callback) {
        this->callback_ = callback;
        return *this;
    };

    SHESDResult fit(const std::vector<float>& series, size_t period);
};
